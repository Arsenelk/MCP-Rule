...
15. In Windows Shell MCP commands, avoid directory arguments ending in `\` immediately before more options; remove the trailing slash, use `/`, or pass explicit files.
16. When reviewing descriptive documents, baseline against the current on-disk source state, not the "background/current status" section of the task text; task text may predate later approved changes.
